		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case JMP:
					...
					if((COND == 0)                       	                      // NO COND
							|| (FR[GREATER]==1 && (COND==7))                            // GREATER
							|| ((FR[EQUAL]==1 || FR[GREATER]==1) && (COND==9))              // GREATER EQUAL
							|| (FR[LESSER]==1 && (COND==8))                            // LESSER
							|| ((FR[EQUAL]==1 || FR[LESSER]==1) && (COND==10))             // LESSER EQUAL
							|| (FR[EQUAL]==1 && (COND==1))                            // EQUAL
							|| (FR[EQUAL]==0 && (COND==2))                            // NOT EQUAL
							|| (FR[ZERO]==1 && (COND==3))                            // ZERO
							|| (FR[ZERO]==0 && (COND==4))                            // NOT ZERO
							|| (FR[CARRY]==1 && (COND==5))                            // CARRY
							|| (FR[CARRY]==0 && (COND==6))                            // NOT CARRY
							|| (FR[ARITHMETIC_OVERFLOW]==1 && (COND==11))                           // OVERFLOW
				...
				case CALL:
					...
					if( (COND == 0) 											  // NO COND
							|| (FR[GREATER]==1 && (COND==7))                      // GREATER
							|| ((FR[EQUAL]==1 || FR[0]==1) && (COND==9))  		  // GREATER EQUAL
							|| (FR[LESSER]==1 && (COND==8))                       // LESSER
							|| ((FR[EQUAL]==1 || FR[1]==1) && (COND==10)) 		  // LESSER EQUAL
							|| (FR[EQUAL]==1 && (COND==1))                        // EQUAL
							|| (FR[EQUAL]==0 && (COND==2))  					  // NOT EQUAL
							|| (FR[ZERO]==1 && (COND==3))  						  // ZERO
							|| (FR[ZERO]==0 && (COND==4))  						  // NOT ZERO
					...
					{ 						  
						...
						/*
									Quais serão implementados:
									- CALL NOT NEGATIVE
									- CALL GREATER EQUAL ZERO
									- CALL LESSER EQUAL ZERO
									- CALL EQUAL ZERO
									-
						*/
				...
				case ZJMP:
					...
					/*
					0 = Greater than Zero
					1 = Greater or Equal than Zero
					2 = Less than Zero
					3 = Less or Equal than Zero
					*/
...
void le_arquivo(void){
	...
	if ( (stream = fopen("TesteCPU.mif","r")) == NULL)  // Abre o arquivo para leitura
	{
